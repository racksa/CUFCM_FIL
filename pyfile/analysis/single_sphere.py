...
ax.plot(sangani_x_list, sangani_y_list, c='black', label='Sangani & Acrivos(1982)')
ax.scatter(c_13_list, vw_list, facecolors='none', edgecolors='black', label='Rigid sphere')
...
ax2.plot(c_13_list, np.abs(vw_list - sangani(c_13_list))/sangani(c_13_list), marker='+', color='black', label='Rigid sphere')
ax2.set_yscale('log')
ax2.set_xscale('log')
ax2.set_xlabel(r'$c^{1/3}$')
ax2.set_ylabel(r'$\% error$')

fig1.tight_layout()
fig2.tight_layout()
# plt.savefig('fig/single_sphere.eps', format='eps')
fig1.savefig('fig/sangani1982.pdf', format='pdf')
fig2.savefig('fig/sangani1982_error.pdf', format='pdf')
plt.show()
